...
STOPWORDS = {
    ...
    "would", "you", "your", "yours", "yourself", "yourselves", "see", "saw",
    "seen", "look", "looked", "looking", "thing", "things", "something",
...
def build_geocode_lookups():
    """
    Build city-state and city-only lookup dicts from geonamescache,
    injecting manual overrides for cities missing or misnamed in the cache.
    ...
    for v in all_cities.values():
        if v["countrycode"] == "US":
            ...
            # Keep highest-population match for city-only fallback
...
def geocode_city(city, state, country, city_state_lookup, city_only_lookup):
    ...
    Only geocodes US records; non-US rows get empty strings and 'non_us'.
    Falls back to city-only lookup if state match fails.
    """
    if str(country).strip() != "USA":
        return "", "", "non_us"
...
def validate_state(state, country):
    """
    Validate state/province code against known US and Canadian codes.
    Returns the original code if valid, 'Unknown' otherwise.
    Prevents foreign region names (e.g. 'Rizal', 'England') from leaking
    into the State column for non-US/Canada records.
    """
...
def engineer_rows(rows, city_state_lookup, city_only_lookup):
    ...
    for row in rows:
        ...
        loc_key     = build_location_key(row.get("City"), row.get("State"), row.get("Country"))
    ...
    for row in rows:
        ...
        # Geocode US records; non-US get empty lat/lon
        lat, lon, geo_method = geocode_city(
            row.get("City"), state, country,
            city_state_lookup, city_only_lookup,
        )
        ...
        out = {
            ...
            "is_night":       1 if occurred_dt and (occurred_dt.hour >= 20 or occurred_dt.hour <= 5) else 0,
            ...
            "report_lag_days": (
                ...
                if occurred_dt and reported_date else ""
            ),
...
def main(
    ...
    us_total   = sum(v for k, v in geo_counts.items() if k != "non_us")
    geocoded   = geo_counts.get("state_match", 0) + geo_counts.get("city_fallback", 0)
    logger.info(f"  Geocode breakdown : {dict(geo_counts)}")
    logger.info(f"  US geocode rate   : {geocoded / us_total * 100:.1f}%" if us_total else "  No US rows")
